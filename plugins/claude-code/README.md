...
2. **Starts backend-specific indexing.** With Milvus Server, launches `memsearch watch .memsearch/memory/` as a singleton background process. A watcher-held filesystem lock prevents duplicates even when process inspection is unavailable; the PID file remains for lifecycle compatibility. The watcher monitors markdown files and auto-re-indexes on changes with a 1500ms debounce. Milvus Lite cannot share its local database with a watcher, so SessionStart launches one background `memsearch index` attempt instead.
